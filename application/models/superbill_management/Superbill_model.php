...
class Superbill_model extends \Mobiledrs\core\MY_Models {
	...
	public function get_transaction_unbill(
		string $fromDate, 
		string $toDate,
		array $type_of_visit = [], 
		string $type,
	...
	) : array
	{
		...
		$transaction_params = [
			...
			'where' => [
				[
					'key' => 'patient_transactions.'.$billed_columns[$type],
					'condition' => '>=',
	        		'value' => $fromDate
        		],
        		[
	...
	public function get_transaction_unbill_details(
		string $fromDate, 
		array $type_of_visit = [], 
		string $type,
		array $ids, 
	...
	public function get_CPO_unbill(string $fromDate, string $toDate, array $ids) : array
	{
	...
	public function get_CPO_unbill_details(string $fromDate, array $ids) : array
	{
		$new_fromDate = strpos($fromDate, '/') !== FALSE ?
			date_format(date_create($fromDate), 'Y-m-d') :
			date_format(date_create(str_replace('_', '/', $fromDate)), 'Y-m-d');
		...
		$dateFormatter = new Date_formatter();
		$dateFormatter->set_date($new_fromDate, $new_fromDate);
		$datePeriod = $dateFormatter->format();
