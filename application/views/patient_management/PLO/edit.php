               ...
               <div class="box-body">
                  ...
                  <div class="row">
                     ...
                     <div class="xrx-info">
                        ...
                        <input type="hidden" name="lab_order_file" value="{{ lab_order.lab_order_file }}">

                              {% if lab_order.lab_order_file %}
										{% set datas = lab_order.lab_order_file|split(',') %}
										{% for data in datas|slice(0, 5) %}

										<input type="hidden" class="{{ data|replace({'"': ''})|replace({'[': ''})|replace({']': ''})|replace({'.': ''})|replace({'(': ''})|replace({')': ''}) }}" value="{{ data|replace({'"': ''})|replace({'[': ''})|replace({']': ''}) }}" name="lab_order_file[]">
										{% endfor %}
                     ...
                     <div class="form-container">
                           ...
                           <div style="margin-bottom: 15px !important;" class="col-md-6 form-group {{ form_error('date_referral') ? 'has-error' : '' }}">
                              ...
                              <input type="text" required="true" class="form-control" data-inputmask="'alias': 'mm/dd/yyyy'" data-mask  name="pt_dateRefEmailed" value="{{ set_value('pt_dateRefEmailed', lab_order.get_date_format(lab_order.date_referral)) }}">
                           </div>
